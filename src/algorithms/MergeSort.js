import { mergeSortActions } from "../store/mergeSortSlice";
...
function handleDispatch(toDispatch, dispatch, array, speed) {
  if (!toDispatch.length) {
    dispatch(mergeSortActions.setCurrentMergeX(array.map((num, index) => index)));
    setTimeout(() => {
      dispatch(mergeSortActions.setCurrentMergeX([]));
  ...
  let dispatchFunction = toDispatch[0].length > 3 ?
    arrayActions.setArray : toDispatch[0].length === 3 && typeof toDispatch[0][2] === "boolean" || toDispatch[0].length === 0 ?
    swapperActions.setCurrentSwappers : toDispatch[0].length === 2 && typeof toDispatch[0][0] === "boolean" ?
    sortedActions.setCurrentSorted : mergeSortActions.setCurrentMergeX;
  if (dispatchFunction === arrayActions.setArray) {
    let currentToDispatch = toDispatch.shift();
    dispatch(dispatchFunction(currentToDispatch.slice(0, currentToDispatch.length - 2)));
    dispatch(swapperActions.setCurrentSwappers([]));
    dispatch(mergeSortActions.setCurrentMergeX([]));
    dispatch(swapperActions.setCurrentSwappers([currentToDispatch[currentToDispatch.length - 2], currentToDispatch[currentToDispatch.length - 1]]));
    dispatch(mergeSortActions.setCurrentMergeX([currentToDispatch[currentToDispatch.length - 2], currentToDispatch[currentToDispatch.length - 1]]));
  } else {
